 ...
 if ((comment_style !==false)&&(comment_style !==undefined)&&(comment_style !== '') ){
   ...
   wholePost += "<label for=\"request-for\" class=\"sr-only\">request belongs to</label>"+
    ...
    wholePost += "<section class=\"col-xs-3 text-muted no-margin post-comment \" id=\""+
                                                   commentid+"\">"+comment_txt+"</section></div></section>";

  wholePost +="<div id=\"cmt-wall\" class=\"col-xs-12 myschool_wall re\"> <ul id=\"cmt-posts-"+commentid+"\" class=\" re each-cmt no-margin no-padding\">"+
      "<span id=\"post-"+ commentid+"\"></span><input type=\"hidden\" name =\""+commentid+"\" id=\"cmt-lastlistid-"+commentid+"\" style=\"display:none;\" value= \"0\"></ul></div>";    
    ...
    $('.post-comment').click(function(){ 
     ...
     // };
      ...
      if (parseInt(postid , 10) <checkLastPostId()+2) {
        $('span#post-'+postid).append(for_comment(postid));
